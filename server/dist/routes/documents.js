...
import { createDocumentForUser, getDocumentForUser, listDocumentsForUser, putDocumentForUser, } from '../db.js';
import { validatePutDocumentBody } from '../middleware/validation.js';
import { z } from 'zod';
const router = Router();
function createDefaultDocument(id) {
    return {
        id,
        sheets: [
...
router.post('/', async (req, res) => {
    const r = req;
    const title = typeof req.body?.title === 'string' ? req.body.title.slice(0, 200) : 'Untitled spreadsheet';
    const id = newDocId();
    const doc = { ...createDefaultDocument(id), id, name: title };
...
router.post('/:id/import/csv', async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ message: 'Invalid CSV import payload' });
    ...
    const nextDoc = { ...doc, sheets: nextSheets };
...
router.put('/:id', validatePutDocumentBody, async (req, res) => {
    const r = req;
    const id = req.params.id;
